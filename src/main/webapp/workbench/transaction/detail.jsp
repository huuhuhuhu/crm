<%@ page import="com.jiabin.crm.settings.domain.DicValue" %>
<%@ page import="java.util.List" %>
<%@ page import="java.util.Map" %>
<%@ page import="java.util.Set" %>
<%@ page import="com.jiabin.crm.workbench.domain.Tran" %>
...
request.getContextPath() + "/";
	...
	for (int i=0;i<dvList.size();i++){
		...
		//根据阶段名称取得相应的可能性（possibility）
		String possibility=pMap.get(stage);

		//如果可能性为零，说明找到了前面正常阶段和垢面丢失阶段的分界点
...
<script type="text/javascript">
	...
	function changeIcon(stage,index1) {
		...
		//当前阶段可能性，不能够从请求对象中取，因为局部刷新，请求对象中的值没有发生改变
		var currentPossibility=$("#possibility").html();

		//当前阶段的下标
		var index=index1;

		//前面正常阶段和后面丢失阶段的分界点下标,只有Java的基本类型才能这样转，基本加字符串
		var point="<%=point%>";
		...
		//如果当前阶段的可能性不为0，前七个绿色标记，黑圈，后两个一定是黑叉
		else{
			//遍历前七个绿色标记，黑圈
			for(var i=0;i<point;i++){
				//如果是当前阶段
