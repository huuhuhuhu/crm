...
<script type="text/javascript">
	$(function(){
		...
		//为提交（市场活动）按钮绑定事件，填充市场活动源（填写两项信息，名字和id）
		...
		/*$("#convertBtn").click(function () {
			/!*
				...
				根据为“为用户创建交易”的复选框有没有选中。来判断是否需要创建交易

			 *!/
			//需要创建交易
			if ($("#isCreateTransaction").prop("checked")){
				//如果需要创建交易，除了要为后台传递clueId之外，还得为后台传递交易表单中的信息，金额，预计成交日期，交易名称，阶段，市场活动源
				//window.location.href=workbench/clue/convert.do？clueId=$param.id&money=xxx&ecpectedDate=xxx......;

				//以上传递参数的方式很麻烦，而且表单一旦扩充，挂载的参数有可能超出浏览器地址栏的上限
				//我们想到使用提交交易表单的形式来发出本次的传统请求
				//提交表单的参数不用我们手动挂载（表单中写name属性），提交的值是name属性对应的value值，提交表单能够提交post请求，不用担心请求参数长度的问腿
